        ...
        <div className="p-6">
          {done ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-[#00255a]">Full Name *</label>
                <input
                  name="name"
                  required
                  value={form.name}
                  ...
                  className="border border-[#00255a] rounded-4xl w-full py-2 px-3 text-sm outline-none focus:ring-2 focus:ring-[#00255a]/20"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#00255a]">Email *</label>
                  <input
                    name="email"
                    type="email"
                    required
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#00255a]">Phone *</label>
                  <input
                    name="phone"
                    type="tel"
                    required
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-[#00255a]">Years of Experience</label>
                <div className="flex items-center overflow-hidden rounded-4xl border border-[#00255a] focus-within:ring-2 focus-within:ring-[#00255a]/20">
                  <input
                    name="experience"
                    type="number"
                    min="0"
                    ...
                    onChange={handleChange}
                    placeholder="0"
                    className="w-full py-2 pl-4 text-sm text-[#00255a]"
                  />
                </div>
              </div>

              <div>
                <label className="mb-1.5 block text-sm font-medium text-[#00255a]">Cover Message</label>
                <textarea
                  name="message"
                  value={form.message}
                  onChange={handleChange}
                  ...
                  className="border border-[#00255a] rounded-2xl w-full py-2 px-3 text-sm outline-none focus:ring-2 focus:ring-[#00255a]/20 resize-none"
                />
              </div>
              ...
              <div>
                ...
                <button
                  ...
                  className="flex w-full cursor-pointer items-center gap-2 rounded-4xl border border-dashed border-[#00255a] px-4 py-3 text-sm text-[#00255a] transition-colors hover:bg-[#00255a]/5"
                >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full cursor-pointer rounded-4xl bg-[#00255a] py-3 text-sm font-semibold text-[#D9B872] shadow-lg shadow-[#00255a]/20 transition-colors hover:bg-[#00255a] disabled:cursor-not-allowed disabled:opacity-60"
              >
...
function JobCard({ career, index, onApply }: { career: Career; index: number; onApply: () => void }) {
  return (
    <motion.div
      ...
      className="group flex flex-col gap-4 rounded-2xl border border-navy-900/8 bg-white p-6 shadow-[0_16px_40px_-28px_rgba(0,37,90,0.35)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_28px_56px_-28px_rgba(0,37,90,0.4)]"
    >
      <div className="flex items-start justify-between gap-4">
        <h3 className="text-lg font-bold leading-snug text-[#00255a] transition-colors group-hover:text-[#00255a]">
          {career.title}
        </h3>
        {career.type && (
          <span className="shrink-0 rounded-full border border-[#00255a]/15 px-3 py-0.5 text-xs font-semibold uppercase tracking-wider text-[#00255a]">
            {career.type}
          </span>
        )}
      </div>
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          {!loading && activeJobs.length > 0 && (
            <motion.div
              ...
              className="mt-14 grid grid-cols-3 gap-x-6 gap-y-6 border-t border-[#00255a]/8 pt-10"
            >
              {[
                ...
                <div key={stat.label} className="relative border-l border-[#00255a]/10 pl-5 first:border-l-0 first:pl-0">
                  <span className="font-serif block text-3xl font-bold leading-none text-[#00255a] md:text-4xl">
                    {stat.value}
                  </span>
